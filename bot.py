...
async def manu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text_manu =='Информацияℹ️':
        await update.message.reply_text(info_string,reply_markup=ReplyKeyboardMarkup(manu_buttons,resize_keyboard=True))
        return MANU
...
async def marketingstbutton(update:Update,context:ContextTypes.DEFAULT_TYPE) ->int:
    ...
    reply_keyboard = [['Проектная работа для дизайнеров','Для Терр. Менеджеров'],['⬅️ Назад']]
